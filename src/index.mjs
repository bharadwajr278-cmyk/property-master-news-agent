...
const relevance = /real estate|property|properties|realty|builder|developer|housing|residential|commercial|rera|project|land|launch|metro|road|expressway|highway|flyover|underpass|airport|rrts|namo bharat|infrastructure|authority|master plan|circle rate|stamp duty|registry|township|corridor|sewer|drainage|water supply|m3m|sobha|shobha|amolik|godrej|dlf|prestige|bptp|ace group/i;
const headlineRelevance = /real estate|property|properties|realty|builder|developer|housing|residential|commercial|rera|project|plot|land|launch|metro|road|expressway|highway|flyover|underpass|airport|rrts|namo bharat|infrastructure|master plan|circle rate|stamp duty|registry|township|corridor|sewer|drain|m3m|sobha|shobha|amolik|godrej|dlf|prestige|bptp|ace group/i;
...
for (const source of sources) {
  ...
  for (const item of items.slice(0, 50)) {
    ...
    if (!title || !description || /\?\s*$/.test(title) || !headlineRelevance.test(title) || !relevance.test(text) || !positiveDevelopment.test(text) || rejection.test(text) || civicProblemOrSpeculation.test(text) || negativeDevelopment.test(text)) { report.skipped.push({ source: source.publisher, title, reason: "not relevant positive real-estate/infrastructure news" }); continue; }
