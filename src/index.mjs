...
const relevance = /real estate|property|properties|realty|builder|developer|housing|residential|commercial|rera|project|land|launch|metro|road|expressway|highway|flyover|underpass|airport|rrts|namo bharat|infrastructure|authority|master plan|circle rate|stamp duty|registry|township|corridor|sewer|drainage|water supply|revamp|repair|renovat|rehabilitat|upgrade|proptech|property technology|real estate startup|construction technology|m3m|sobha|shobha|amolik|godrej|dlf|prestige|bptp|ace group/i;
const headlineRelevance = /real estate|property|properties|realty|builder|developer|housing|residential|commercial|rera|project|plot|land|launch|metro|road|expressway|highway|flyover|underpass|airport|rrts|namo bharat|infrastructure|master plan|circle rate|stamp duty|registry|township|corridor|sewer|drain|revamp|repair|renovat|rehabilitat|upgrade|proptech|property technology|real estate startup|construction technology|m3m|sobha|shobha|amolik|godrej|dlf|prestige|bptp|ace group/i;
const positiveDevelopment = /launch|acquir|purchase|land deal|development agreement|joint development|partnership|signs? (?:an? )?(?:mou|agreement)|invest|funding|fundraise|raises? (?:₹|rs|inr|\$)|proposal|propos(?:e|es|ed|ing)|plans?|directs?|targets?|set to|to develop|approval|approv(?:es|ed|al)|clears?|sanction|tender|construction (?:starts|begins|completed)|work (?:starts|begins|completed)|inaugurat|commission|new (?:project|road|metro|corridor|flyover|underpass|expressway|housing)|expand|extension|upgrade|upgradation|overhaul|redevelop|revamp|repair|renovat|rehabilitat|multimodal|multi-modal|connectivity|milestone|rera registration|possession|handover|sales bookings?|pre-sales|booking value|complet(?:e|ed|ion)/i;
const rejection = /murder|assault|robbery|arrest|\bfirs?\b|corruption|bribery|bribe|scam|fraud|forgery|cheating case|criminal investigation|vigilance (?:probe|raid|case)|accident|suicide|killed|\bdies\b|\bdied\b|death|injured|crash|collision|collides?|vehicle\s+.*\brams?\b|\brams?\s+into\b|\bhits?\s+(?:a\s+)?(?:pole|divider)\b|power\s*cut|power\s+outage|without\s+(?:electricity|power)|electrocution|stunt|viral|police|gangster|liquor|pilgrim|devotee|school bus|biryani|sanitation strike|garbage heap|rain havoc|traffic snarl|missing (?:streetlights?|streetlight poles?|lights?)|(?:road|street).{0,60}(?:in darkness|dark)|in darkness|without (?:streetlights?|lighting)|non-functional streetlights?|unsafe road|blackout|two years on|horoscope|election|celebrity|sports|lifestyle/i;
...
for (const source of sources) {
  ...
  for (const item of items.slice(0, 50)) {
    ...
    if (/\bgreater noida\b/i.test(text)) { report.skipped.push({ source: source.publisher, title, reason: "Greater Noida is not an approved Noida feed location" }); continue; }
